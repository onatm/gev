...
def _metadata(tmp_path, **updates):
    ...
    value = {"model_name": "google/gemma-3-1b-pt", "model_revision": "r",
             "base_model_type": "gemma3_text", "lora": c.LORA_CONTRACT,
             "head_width": 256,
             "adapter_sha256": hashlib.sha256(b"adapter").hexdigest(),
             "pointer_sha256": hashlib.sha256(b"pointer").hexdigest(),
             "marker_ids": {"state": 1, "question": 2, "option_start": 3, "option_end": 4, "decide": 5},
             "marker_strings": {"state": "s", "question": "q", "option_start": "os", "option_end": "oe", "decide": "d"},
             "tokenizer_revision": "r"}
    value.update(updates)
    (tmp_path / "metadata.json").write_text(json.dumps(value))
    return value


def _config():
    return SimpleNamespace(model=SimpleNamespace(name="google/gemma-3-1b-pt", revision="r"))
...
def test_audit_counts_state_in_branch_limit(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("gev.tokenization.MarkerMap.load", lambda *args: object())
    monkeypatch.setattr("gev.tokenization.rows_of", lambda encoded: ([], [], [{"ids": [0] * 7}]))
    ...
    config = SimpleNamespace(model=SimpleNamespace(name="x", revision="r"), training=SimpleNamespace(
        seed=1, p_none=.1, p_none_distract=.1, p_distract=.1, p_none_pair=.1,
        state_cap=10, branch_cap=10, packed_cap=20))
