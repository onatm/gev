...
def protocol(tmp_path, monkeypatch):
    ...
    meta = {
        "temperature": 1.0,
        "training": {"complete": True, "logical_steps": 3144, "processed_records": 25152, "source_count": 12576},
        "source_sha256": locked.V7_TRAIN_SHA256, "manifest_sha256": locked.V7_MANIFEST_SHA256,
        "config": {"experiment_id": "gemma3-1b-v7", "training": {"epochs": 2, "logical_batch": 8,
                   "context_length": 384, "seed": 0}},
    }
    selection = {
...
def test_wrong_fingerprint_and_incomplete_lineage_are_preflight_failures(protocol):
    ...
    incomplete = {**meta, "training": {**meta["training"], "logical_steps": 32}}
...
def test_normal_cli_split_loader_refuses_test(monkeypatch, tmp_path):
    from gev.cli import _load_cli_split
    from gev.data.suites import SuiteError
    monkeypatch.setattr("gev.cli.load_manifest", lambda _: {})
    with pytest.raises(SuiteError, match="locked"):
        _load_cli_split(str(tmp_path), "decision-v7", "test", allow_test=True)
...
def _checkpoint_fixture(root, *, seed=0, smoke=False):
    ...
    meta = {
        "model_name": "fixture", "model_revision": "fixture", "marker_ids": {},
        "marker_strings": {}, "marker_bos": False, "adapter_sha256": locked._sha(checkpoint / "adapter_model.safetensors"),
        "pointer_sha256": locked._sha(checkpoint / "pointer.safetensors"), "temperature": 1.0,
        "source_sha256": locked.V7_TRAIN_SHA256, "manifest_sha256": locked.V7_MANIFEST_SHA256,
        "training": {"complete": True, "source_count": 12576, "processed_records": 25152, "logical_steps": 3144},
        "config": {"experiment_id": "gemma3-1b-v7", "training": {"seed": seed, "epochs": 2,
                   "logical_batch": 8, "context_length": 384, "p_none_pair": .25}},
    }
    if smoke:
        meta["continuation"] = {"diagnostic_smoke_init": True}
    (checkpoint / "metadata.json").write_text(json.dumps(meta))
...
def test_nested_smoke_continuation_is_rejected_before_reservation(protocol):
    root, selection, _ = protocol
    meta = {**protocol[2], "continuation": {"diagnostic_smoke_init": True}}
