...
def test_local_predictor_rejects_nonfinite_or_nonpositive_temperature(temperature):
    with pytest.raises(ValueError): TorchPredictor(object(), object(), object(), state_cap=10, branch_cap=10, packed_cap=10, temperature=temperature)
...
def test_local_predictor_rows_and_evaluation_apply_temperature_once(tmp_path):
    ...
    predictor = TorchPredictor(model, SyntheticTokenizer(), markers, state_cap=64, branch_cap=128, packed_cap=256, temperature=2.0)
