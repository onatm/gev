...
from ...domain.tokenization import MarkerMap, encode, rows_of
...
_TARGETS = GEMMA3_TEXT.lora_targets
...
def build_tiny_model(*, layers: int = 6, hidden_size: int = 64, window: int = 8,
    ...
    torch.manual_seed(0)
    return resolve_model("gemma3_text", "torch").create_model(
        model_name="tiny-diagnostic", revision="random",
        backbone=Gemma3TextModel(_tiny_config(layers, hidden_size, window)),
...
def _run_diagnostic(model: GemmaRowModel, *, pretrained: bool, model_name: str,
                    ...
                    device: str | None = None, encoder=encode) -> dict[str, Any]:
    ...
    if tokenizer is None:
        ...
        encoded_record = encoder(tokenizer, materialize(_diagnostic_record()), markers,
