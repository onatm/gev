...
fresh ``gev train``/``gev eval`` child processes so one GPU and one model are
...
from .data.suites import REQUIRED_FILE_HASHES, load_manifest, load_split
...
def _verified(root: Path, suite: str, split: str) -> tuple[list[dict], dict]:
    ...
    if child.exists():
        ...
        if not manifest.get("smoke_only") or manifest.get("parent", {}).get("suite") != suite:
            raise ValueError("custom suite is not a verified smoke child")
        path = root / f"{split}.jsonl"
        return load_split(path, suite, split, manifest), manifest
    manifest = load_manifest(suite)
    path = root / suite / f"{split}.jsonl"
    return load_split(path, suite, split, manifest), manifest
...
def plan(config_path: str | Path, *, seeds=(0, 1, 2), data="data", smoke=False) -> dict:
    ...
    recipe["training"].pop("seed", None)
    recipe["training"].pop("save_every", None)
    recipe_hash = _sha_bytes(json.dumps(recipe, sort_keys=True, separators=(",", ":")).encode())
    ...
    return {"status": "planned", "config": str(config_path), "config_sha256": recipe_hash,
            ...
            "model": {"name": config.model.name, "revision": config.model.revision, "dtype": config.training.dtype,
...
def _toml_for_seed(config_path: Path, seed: int, directory: Path, *, save_every=None) -> Path:
    text = config_path.read_text(encoding="utf-8")
    lines = text.splitlines()
    in_training = False
    has_save_every = False
    training_end = len(lines)
    for i, line in enumerate(lines):
        if line.strip().startswith("["):
            if in_training:
                training_end = i
            in_training = line.strip() == "[training]"
        if in_training and line.strip().startswith("seed") and "=" in line:
            lines[i] = f"seed = {seed}"
        if in_training and line.strip().startswith("save_every") and "=" in line:
            has_save_every = True
    if in_training:
        training_end = len(lines)
    if save_every is not None and not has_save_every:
        lines.insert(training_end, f"save_every = {save_every}")
    path = directory / f"seed-{seed}.toml"; path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return path
...
def _run_child(command, *, log_path: Path, on_update, heartbeat_interval=15.0,
...
def run(config_path: str | Path, *, seeds=(0, 1, 2), data="data", out="runs/study",
    ...
    for index, seed in enumerate(seeds, 1):
        ...
        cfg = _toml_for_seed(config_path, seed, configs,
        ...
        def execute(stage, command):
            ...
            try:
                child = _run_child(command, log_path=log_path, on_update=update,
        ...
        try:
            if existing_run:
                command = [sys.executable, "-m", "gev", "eval", "--run", existing_run,
                           "--config", str(cfg), "--suite", "decision-v7", "--split", "development",
                           "--data", data, "--temperature", "1", "--out", str(trial / "development")]
                stage_result = execute("development", command)
                stages = [stage_result]
            else:
                train_cmd = [sys.executable, "-m", "gev", "train", "--config", str(cfg),
                ...
                if stages[-1]["outcome"] == "completed":
                    for name, suite in (("calibration", "decision-v7"),
                        ...
                        eval_cmd = [sys.executable, "-m", "gev", "eval", "--run", str(trial),
                                    "--config", str(cfg), "--suite", suite, "--split", split,
