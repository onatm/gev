"""Scoring of benchmark rows, adapted from Kev.

Source: https://github.com/jaredpalmer/kev/blob/08ab0b87d27cb5577a3b371ad7ed4e4686b0502b/kev/metrics.py
Copyright (c) the Kev contributors.  Adapted for pure NumPy and Gev row dicts.
"""
import math
...
def nll(row, temperature=1.0):
    """Compatibility name retained for existing Gev callers."""
    return nll_at_temperature(row, temperature)


def _row_scores(row):
    p = np.asarray(row["p"], dtype=float); y = row["label"]
    conf, ok = float(p.max()), bool(p.argmax() == y)
    return {"acc": float(ok), "nll": nll_at_temperature(row),
            "brier": float(((p - np.eye(len(p))[y]) ** 2).sum()), "mean_conf": conf,
            "confident_error_rate": float(conf >= 0.9 and not ok),
            "coverage_at_0_9": float(conf >= 0.9), "confidence_bias": conf - float(ok)}
...
def metrics(rows, temperature=1.0):
    ...
    for row in rows:
        p = probabilities_at_temperature(row, temperature); y = row["label"]
        target = np.eye(len(p))[y]
        nlls.append(nll_at_temperature(row, temperature)); acc.append(int(p.argmax() == y))
    ...
    result = {"n": len(rows), "nll": float(np.mean(nlls)), "acc": float(np.mean(acc)),
    ...
    result.update(confident_error_rate=float(np.mean(high & ~correct)), coverage_at_0_9=float(high.mean()),
                  ...
                  top_bins={str(t): {"n": int((confidence >= t).sum()), "errors": int(((confidence >= t) & ~correct).sum()),
                                    "error_rate": float((~correct[confidence >= t]).mean()) if (confidence >= t).any() else None}
...
def _risk_curve_arrays(confidence, correct):
    ...
    if not len(confidence): return confidence, np.array([], dtype=int), np.array([], dtype=int)
    order = np.argsort(-confidence, kind="stable"); confidence, errors = confidence[order], np.cumsum(~correct[order])
...
def area_under_risk_coverage(confidence, correct):
    _, accepted, errors = _risk_curve_arrays(confidence, correct)
    if not len(accepted): return 0.0
...
def unknowable_report(rows):
    unk = [r for r in rows if r["source"] == "unknowable"]
    ctl = [r for r in rows if r["source"] == "unknowable_control"]
    ...
    return {"n": len(unk), "mean_max_p": float(np.mean(conf(unk)),), "share_at_0_9": float(np.mean([c >= .9 for c in conf(unk)])),
            "control_mean_max_p": float(np.mean(conf(ctl))) if ctl else None,
            "control_share_at_0_9": float(np.mean([c >= .9 for c in conf(ctl)])) if ctl else None,
...
def raw_row(row, temperature=None):
    served = row.get("inference_temperature", 1.0) if temperature is None else temperature
    _check_temperature(served)
    if served == 1.0: return row
    if "logits" not in row: raise ValueError("cannot restore raw logits from a row that recorded none")
    z = np.asarray(row["logits"], dtype=float) * served; p = np.exp(z - z.max())
...
def fit_temperature(rows, aggregation="macro", points=81):
    if aggregation not in ("micro", "macro"): raise ValueError("invalid calibration aggregation")
    clean = scored_rows(rows)
    if not clean: raise ValueError("cannot fit temperature without labelled calibration rows")
    ...
    candidates = np.exp(np.linspace(np.log(.25), np.log(4), points)); weights = np.ones(len(clean))
    if aggregation == "macro":
        counts = defaultdict(int)
        for row in clean: counts[row["task"]] += 1
...
def _source_groups(rows):
    groups = defaultdict(list)
    for i, row in enumerate(rows): groups[(row["source"], row["group"])].append(i)
    sources = defaultdict(list)
    for (source, _), indices in groups.items(): sources[source].append(np.asarray(indices, dtype=int))
    return sources


def grouped_folds(rows, folds, seed):
    units = sorted({(row["source"], row["group"]) for row in rows})
    if len(units) < folds: raise ValueError("fewer distinct (source, group) units than folds")
    rng = np.random.default_rng(seed); fold_of, offset = {}, 0
    for source in sorted({source for source, _ in units}):
        own = [unit for unit in units if unit[0] == source]
        for i, j in enumerate(rng.permutation(len(own))): fold_of[own[j]] = (offset + i) % folds
        offset += len(own)
...
def out_of_fold_rows(rows, folds=5, seed=0, **fit_kwargs):
    ...
    clean = scored_rows(rows); fold_id = grouped_folds(clean, folds, seed)
    temperatures, oof = [], list(clean)
    for fold in range(folds):
        held = fold_id == fold
        temperature = fit_temperature([row for row, out in zip(clean, held) if not out], **fit_kwargs)
        temperatures.append(temperature)
        for i in np.flatnonzero(held): oof[i] = tempered_row(clean[i], temperature)
    return oof, temperatures


def cross_validated_temperature(rows, folds=5, seed=0, samples=1000, **fit_kwargs):
    if samples < 1: raise ValueError("samples must be >= 1")
    clean = scored_rows(rows); oof, temperatures = out_of_fold_rows(clean, folds, seed, **fit_kwargs)
    keys = ("n", "ece", "brier", "nll", "confident_error_rate", "coverage_at_5pct_error")
    raw, out_of_fold = metrics(clean), metrics(oof)
    correct = np.asarray([np.argmax(row["p"]) == row["label"] for row in clean])
    conf = np.asarray([[max(row["p"]) for row in group] for group in (clean, oof)])
    sources = _source_groups(clean); rng = np.random.default_rng(seed); values = np.empty((samples, 2))
    ...
    ci = {name: np.quantile(v, [.025, .975]).tolist() for name, v in (("raw", values[:, 0]), ("out_of_fold", values[:, 1]), ("delta", values[:, 1] - values[:, 0]))}
    return {"raw": {k: raw[k] for k in keys}, "out_of_fold": {k: out_of_fold[k] for k in keys}, "ece_ci95": ci,
            "separated": bool(ci["delta"][1] < 0 or ci["delta"][0] > 0), "temperatures": temperatures, "folds": folds,
            "seed": seed, "samples": samples, "groups": sum(len(v) for v in sources.values()),
...
def paired_bootstrap(candidate, reference, samples=1000, seed=0, metric="nll", aggregation="macro"):
    ...
    additive = metric not in nonlinear
    ...
    def index(rows):
        ...
        for r in scored_rows(rows):
            key = r["id"], r["question"]
            if key in out: raise ValueError("duplicate paired example")
            out[key] = r
        return out
    a, b = index(candidate), index(reference)
    if not a or a.keys() != b.keys(): raise ValueError("paired comparison requires identical complete clean examples")
    keys = sorted(a)
    for key in keys:
        row, other = a[key], b[key]
        if row["keys"] != other["keys"] or row["label"] != other["label"]: raise ValueError("paired comparison labels or option order differ")
        if any(row[field] != other[field] for field in ("source", "group", "task", "type")): raise ValueError("paired comparison group or task metadata differ")
    sources = _source_groups([a[key] for key in keys]); task_names = np.asarray([a[key]["task"] for key in keys]); statistics = []
    for indexed in (a, b):
        rows = [indexed[key] for key in keys]; conf = np.asarray([max(r["p"]) for r in rows]); correct = np.asarray([np.argmax(r["p"]) == r["label"] for r in rows])
        values = np.asarray([_row_scores(r)[metric] for r in rows]) if additive else None; statistics.append((conf, correct, values))
    def statistic(indices, data):
        conf, correct, values = data
        if values is not None: return float(values[indices].mean())
        conf, correct = conf[indices], correct[indices]
        if metric == "aurc": return area_under_risk_coverage(conf, correct)
        if metric == "ece": return ece(conf, correct)
        return coverage_at_error(conf, correct, .05 if metric == "coverage_at_5pct_error" else .01)
    def delta(indices):
    ...
    return {f"{aggregation}_{metric}_delta": delta(np.arange(len(keys))), "ci95": np.quantile(values, [.025, .975]).tolist(),
